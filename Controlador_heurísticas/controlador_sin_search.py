...
def avoid(sonar):
    ...
    if (sonar[3] < 0.3) or (sonar[4] < 0.3) or (sonar[5] < 0.25) or (sonar[2] < 0.25):
        if lateral_der > lateral_izq:
             lspeed, rspeed = +0.6, -0.2 # Gira derecha
             print("derecha")
        else: 
            lspeed, rspeed = -0.2, +0.6 # Gira izquierda
            print("izquierda")
    elif (sonar[14] < 0.25) or (sonar[12] < 0.3) or (sonar[11] < 0.3) or (sonar[9] < 0.25):
...
def search(last_coord):
    if last_coord[0] <= 0.5: # Pelota hacia la izquierda
        lspeed, rspeed = -0.5, +0.9 # Gira izquierda
    else: #Pelota hacia la derecha
        lspeed, rspeed = +0.9, -0.5 # Gira derecha
